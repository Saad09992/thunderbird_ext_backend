...
const extractTextFromPineconeResponse = (response) => {
  const combinedText = response?.matches
...
*/
    const refrenceEmailsRetrieve = async (state) => {
      let vec = await embeddings.embedQuery(state.question);

      const response = await refrenceIndex.namespace(sessionId).query({
        topK: 5,
        vector: vec,
        includeMetadata: true,
      });
      const textEmails = extractTextFromPineconeResponse(response);
      return { ...state, refrenceEmails: textEmails };
    };
    const datasetRetrieve = async (state) => {
      let vec = await embeddings.embedQuery(state.question);

      const response = await datasetIndex.namespace(sessionId).query({
        topK: 5,
        vector: vec,
        includeValues: true, // ← Must be true to get `values`
        includeMetadata: true,
      });

      const textDataset = extractTextFromPineconeResponse(response);
      return { ...state, dataset: textDataset };
    };
    /*
    const refrenceEmailsRetrieve = async (state) => {
      ...
      console.log("REFRENCE EMAILS: ", retrievedDocs);
    ...
    const datasetRetrieve = async (state) => {
      ...
      console.log("RETRIEVED DATA: ", retrievedDocs);
  ...
  try {
    ...
    for (let i = 0; i < processedDataset.length; i += batchSize) {
      ...
      batchEmbeddings.forEach((vector, idx) => {
        console.log("BATCH TEXT:", batch[idx]);
        vectors.push({
          id: `${sessionId}-${i + idx}`,
          values: vector,
          metadata: {
    ...
    return res
      .status(400)
      .json({ message: "Error processing dataset", error: error.message });
  }
};
...
export const removeDataset = async (req, res) => {
  try {
    ...
    if (stats.namespaces && stats.namespaces[sessionId]) {
      try {
        ...
        return res
          .status(400)
          .json({ message: "Uncatchable Error", error: err.message });
      }
    }
